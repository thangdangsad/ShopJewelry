...
namespace WebTrangSuc.Controllers
{
    public class DongHoController : Controller
    {
        ...
        public ActionResult DongHo(string sortOrder, string searchString, int page = 1)
        {
            int pageSize = 8;

            // Lấy danh sách sản phẩm thuộc loại  DongHo
            var query = db.SanPhams.Where(sp => sp.IDLoaiSp == 2);
